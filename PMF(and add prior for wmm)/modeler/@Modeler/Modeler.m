    ...
    methods %(Access=public)
        ...
        function v=Modeler(args)
            ...
            if  nargin > 0
                ...
                v.lib = args.lib;  %% 添加属性，为了给WMM加入先验分布
        ...
        function v=fitting(v)
            ...
            if strcmp(v.mFittingMethod,'cuckooSearch')
                v=v.cuckooSearch();  % 利用cuckooSearch来拟合模型
            else
                assert(false);
            end
        end
        ...
        function score=computeScore(v,theta)
            ...
            %加入笔画（stroke）；pkappa先验：
            len_S = length(v.mTrainModel.S);
            pkappa_value = v.lib.pkappa(len_S);  

            %加入每一个笔画下面的子笔画（sub-stroke）；pmat_nsub先验：
             %加入笔画关系的概率；rel
            total_motor_prob = 1;
            for i = 1:len_S
                motor_len = length(v.mTrainModel.motor{i, 1});
                motor_prob = v.lib.pmat_nsub(len_S, motor_len);
                total_motor_prob = total_motor_prob * motor_prob; % 将一个笔画下面的所有子笔画的概率取值相加。
            
                relation = v.mTrainModel.S{i,1}.R.type;
                
                if strcmp(relation, 'unihist')
                    total_motor_prob = total_motor_prob * v.lib.rel.mixprob(1);
                elseif strcmp(relation, 'start')
                    total_motor_prob = total_motor_prob * v.lib.rel.mixprob(2);
                elseif strcmp(relation, 'end')
                    total_motor_prob = total_motor_prob * v.lib.rel.mixprob(3);
                elseif strcmp(relation, 'mid')
                    total_motor_prob = total_motor_prob * v.lib.rel.mixprob(4);
                end
            end
 

            %score = init_score * pkappa_value * total_motor_prob ;
             
           lambda = 0.5;  % lambda 控制数据与先验的平衡
           score = init_score * (lambda + (1 - lambda) * pkappa_value * total_motor_prob);
