...
function G = fit_motorprograms(I,K,verbose,include_mcmc,fast_mode)
    ...
    % generate parses
    initMP = generate_random_parses(I,lib,K,verbose); % I:105×105 logical 数组; lib: Library - 属性; K=1; verbose=True      %%%%%%%%%% initMP：初始的MP。后面有一个final_MP
...
%         set(h,'Position',pos);
    ...
    finalMP = cell(nl,1);                          %   finalMP：最后的MP。前面有一个initMP
    for i=1:nl
    ...
    % sort programs from best to worst .   program 有好到坏排列
    [~,score_indx] = sort(final_scores,1,'descend');   % score_indx中的第一个元素对应于每列中得分最高的元素的位置，
    finalMP = finalMP(score_indx);          %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%选取score_index对应的那个finalMP作为finalMP.
