...
function [bestM_pass2,score_sorted] = parses_to_MPs(I,PP,ninit,lib,verbose)
    ...
    nwalk = numel(S_walks);  % 计算S_walks的数量

    % Transform sub-parsed random walks into motor programs
    fclassify = @(traj,scale) classify_traj_as_subid(traj,scale,lib);    % @(traj,scale)表示定义了一个匿名函数，输入参数为traj和scale。这个匿名函数调用classify_traj_as_subid函数，并将traj、scale和lib作为参数传递给该函数。
    MP = cell(nwalk,1);  % 创建了一个34 x 1的一个空的cell数组
    ...
    % Remove all but the most promising parses   .. 删除除最有希望的解析之外的所有解析
