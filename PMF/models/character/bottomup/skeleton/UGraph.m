classdef UGraph < BetterHandle
    ...
    methods
        ...
        % Merge groups of graph nodes connected      用于在图像骨架中合并连接在一起的节点。
        % by the maximum circle criterion
        function clean_skeleton(this)
           I_mask = this.max_circle_mask();  % 调用max_circle_mask方法生成一个二值化掩模I_mask，用于确定哪些节点需要合并
           
           % cluster nodes
           C = this.cluster_G(I_mask);  % 调用cluster_G方法对节点进行聚类，将相互连接的节点分为不同的群组。
           nc = numel(unique(C));
           ...
           while has_rmv(C)
              ...
              for i=1:nc
                  sel = C == i;
                  if sum(sel)>1
                     this.merge(sel,I_mask);   % 使用循环，遍历所有聚类，对每个聚类中的节点进行合并。如果聚类中有超过1个节点，则调用merge方法，将聚类中的节点合并为一个节点。
                     
                     % cluster nodes
                     C = this.cluster_G(I_mask);  % 调用cluster_G方法更新节点聚类，并检查是否还有待合并的节点。如果有，继续执行步骤3；否则，终止循环。
                     [~,nc] = has_rmv(C);
                     break
                  end                  
              end
           ...
           function [yyyy,max_c] = has_rmv(myC)   % has_rmv的嵌套函数，用于检查当前是否有待合并的节点。该函数计算聚类中的唯一值的数量，并将其与节点总数进行比较。如果数量不同，则说明存在待合并的节点。
               myC(myC==0) = [];
